...
for x = 1:2:length(varargin)
    ...
    switch lower(name)
        ...
        case 'clim'
            if ~isempty(value) && ~isequal(numel(value),2)
                close(gcf);
                error(sprintf('%s:invalidOptionValue', app),...
                    'Value for option %s must be a 2-element numeric array',...
                    name);
            elseif ~isempty(value) && ~all(isnumeric(value))
                close(gcf);
                error(sprintf('%s:invalidOptionValue', app),...
                    'Value for option %s must be a 2-element numeric array',...
                    name);
...
set(data_ax,...
    ...
    'tickdir', 'out',...
    'linewidth', 1);
% Have the height of the y-axis for perspective
...
fname_template = [obj.dbds(1).glider...
    '_'...
    datestr(t0,'yyyymmddTHHMM')...
    '-'...
